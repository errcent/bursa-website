...
export type VaporizeTextCycleProps = {
  ...
  tag?: Tag;
  playMode?: "cycle" | "hold-then-vaporize";
  vaporizeWhen?: boolean;
...
export default function VaporizeTextCycle({
  ...
  const vaporizeDoneRef = useRef(false);
  ...
  useEffect(() => {
    if (isInView) {
      const startAnimationTimeout = setTimeout(() => {
        setAnimationState(playMode === "hold-then-vaporize" ? "fadingIn" : "vaporizing");
      }, 0);
  ...
  useEffect(() => {
    if (playMode !== "hold-then-vaporize") return;
    if (!vaporizeWhen || animationState !== "waiting") return;
    vaporizeProgressRef.current = 0;
    resetParticles(particlesRef.current);
    setAnimationState("vaporizing");
  }, [playMode, vaporizeWhen, animationState]);
      ...
      switch (animationState) {
        ...
        case "vaporizing": {
          ...
          if (vaporizeProgressRef.current >= 100 && allVaporized) {
            if (playMode === "hold-then-vaporize") {
              if (!vaporizeDoneRef.current) {
                vaporizeDoneRef.current = true;
                onVaporizeComplete?.();
              }
              setAnimationState("static");
              break;
