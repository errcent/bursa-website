...
import { GuidanceCourseCarousel } from "@/components/learning-guidance/guidance-course-carousel";
import { GuidancePlaylistCarousel } from "@/components/learning-guidance/guidance-playlist-carousel";
    ...
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-12 pb-[env(safe-area-inset-bottom,0px)] sm:gap-14">
          ...
          {profileTags.length > 0 ? (
            <ul className="flex flex-wrap gap-1.5 pt-1">
              {profileTags.map((tag) => (
                <li
                  key={tag}
                  className="rounded-md border border-border/60 bg-surface/40 px-2.5 py-1 text-xs font-medium text-foreground/80"
                >
                  {tag}
                </li>
              ))}
      ...
      <section className="space-y-5">
        ...
        {result.courses.length === 0 ? (
          ...
          <GuidanceCourseCarousel courses={result.courses} />
        )}
      </section>
      ...
      <section className="space-y-5">
        ...
        {playlists.length === 0 ? (
          ...
          <GuidancePlaylistCarousel playlists={playlists} />
        )}
      </section>
