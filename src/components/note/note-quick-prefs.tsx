        ...
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              className={cn(
                "inline-flex min-h-9 coarse:min-h-11 items-center rounded-md border px-2.5 text-xs font-medium transition-colors",
                on
    ...
    <div className={cn("space-y-3", compact ? "px-0.5" : "")}>
      ...
      {copy.usdIdrRate ? (
        <div className="space-y-1">
          ...
          <p className="min-h-9 coarse:min-h-11 w-full rounded-md border border-zinc-800 bg-zinc-900/50 px-2 py-2.5 text-sm tabular-nums text-zinc-300">
