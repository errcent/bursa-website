      ...
      input={
        <LabToolPanel>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <LabField label="Multiplier" id="atr-m" helperText="Umumnya 1,5â€“3Ã- ATR"><LabNumberInput id="atr-m" value={mult} onChange={setMult} min={0} step={0.5} /></LabField>
          </div>
          <div className="mt-4">
      ...
      output={
        <LabOutputPanel footer={<LabCopyResults text={copyText} />}>
          ...
          <LabInterpretation className="mt-3">
            Stop = harga {direction === "long" ? "âˆ’" : "+"} (ATR Ã- {mult}). Ini level saat ini, trailing stop bergerak mengikuti harga.
          </LabInterpretation>
        </LabOutputPanel>
      }
    />
              ...
              <div className="flex h-20 items-end gap-1">
                {histogram.map((bin, i) => (
                  <div
                    ...
                    title={`${fmt(bin.from, 1)}â€“${fmt(bin.to, 1)}: ${bin.count}`}
                  />
                ))}
              </div>
            </div>
