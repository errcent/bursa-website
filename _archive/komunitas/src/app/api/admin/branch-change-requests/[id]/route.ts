import { ChangeRequestStatus, type Prisma } from "@prisma/client";
import { NextResponse } from "next/server";

import { requireAdmin, requireAdminPanel, unauthorized } from "@/lib/admin/server";
import {
  applyChatBranchChangeRequest,
...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    const item = await db.chatBranchChangeRequest.findUnique({
      where: { id },
    ...
    return NextResponse.json(mapChatBranchChangeRequest(item));
  } catch (error) {
    console.error(error);
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const existing = await db.chatBranchChangeRequest.findUnique({ where: { id } });
    if (!existing) {
      return NextResponse.json({ error: "Pengajuan tidak ditemukan." }, { status: 404 });
    }
    ...
    if (decision === "reject") {
      const item = await db.chatBranchChangeRequest.update({
        where: { id },
        data: {
      ...
      await db.adminAuditLog.create({
        data: {
          adminId: admin.id,
          action: "CHAT_BRANCH_CHANGE_REJECT",
          entityType: "chat_branch_change_request",
          entityId: id,
    ...
    const applied = await applyChatBranchChangeRequest(existing, payloadToApply);
    ...
    await db.adminAuditLog.create({
      data: {
        adminId: admin.id,
        action: withEdit
          ? "CHAT_BRANCH_CHANGE_EDIT_APPROVE"
          : "CHAT_BRANCH_CHANGE_APPROVE",
        entityType: "chat_branch_change_request",
        entityId: id,
        changes: { applied },
      },
